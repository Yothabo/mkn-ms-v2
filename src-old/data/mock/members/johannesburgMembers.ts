...
const maleNames = ['Lerato', 'Kagiso', 'Thabiso', 'Katlego', 'Bongani', 'Sipho', 'Tshepo', 'Moses', 'Jacob', 'Samuel'];
const femaleNames = ['Precious', 'Nomsa', 'Zandile', 'Nthabiseng', 'Refilwe', 'Amahle', 'Liyana', 'Nadia', 'Zahara', 'Lerato'];
const surnames = ['Molefe', 'Van der Merwe', 'Botha', 'Zulu', 'Xaba', 'Petersen', 'Williams', 'Brown', 'Smith', 'Johnson'];
...
export const johannesburgMembers: ExtendedMember[] = [];
...
for (let i = 1; i <= 50; i++) {
  ...
  if (i <= 29) status = 'active';
  else if (i <= 37) status = 'preRa';
  else if (i <= 41) status = 'ra';
  else if (i <= 46) status = 'inactive';
  ...
  const member: ExtendedMember = {
    id: `jhb-${i.toString().padStart(3, '0')}`,
    name,
    surname,
    gender,
    dateOfBirth: dob,
    ...
    nextOfKin: {
      ...
      phone: generatePhoneNumber('South Africa'),
      address: `${Math.floor(Math.random() * 100) + 1} ${surnames[Math.floor(Math.random() * surnames.length)]} Street, Johannesburg`
    },
    address: `${Math.floor(Math.random() * 100) + 1} ${surnames[Math.floor(Math.random() * surnames.length)]} Street, Johannesburg`,
    raCount: 0,
    raLock: false,
  ...
  if (status === 'deceased') {
    member.deceasedInfo = {
      ...
      burialPlace: `${Math.floor(Math.random() * 100) + 1} ${surnames[Math.floor(Math.random() * surnames.length)]} Cemetery, Johannesburg`
    };
  }
  ...
  johannesburgMembers.push(member);
}

export default johannesburgMembers;
