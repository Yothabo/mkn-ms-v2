...
import { Position as MemberPosition } from '../config/members';
...
export const testDutyAssignment = () => {
  ...
  if (wednesdayChairDuty) {
    ...
    console.log('Sample eligible member:', eligibleMembers[0] ? 
      `${eligibleMembers[0].name} ${eligibleMembers[0].surname} (${eligibleMembers[0].position}, purity: ${eligibleMembers[0].purity})` : 'None'
    );
  }
